...
export class ApiDataProvider implements PlatformDataProvider {
  ...
  getAutomatedReviewStatus(submissionId: string, signal?: AbortSignal) { return this.get<AutomationRun>(`/api/intake/submissions/${encodeURIComponent(submissionId)}/automated-review/status`, signal); }
  getAutomatedReviewRuns(submissionId: string, signal?: AbortSignal) { return this.get<{ items: AutomationRun[]; message: string }>(`/api/intake/submissions/${encodeURIComponent(submissionId)}/automated-review/runs`, signal); }
  getAutomatedReviewSummary(submissionId: string, signal?: AbortSignal) { return this.get<AutomationSummary>(`/api/intake/submissions/${encodeURIComponent(submissionId)}/automated-review/summary`, signal); }
